...
CColorCopDlg::CColorCopDlg(CWnd *pParent /*=NULL*/)
  ...
  // MAKEINTRESOURCE(IDR_MAINFRAME), IMAGE_ICON, 32, 32, NULL); m_hIcon =
...
ON_UPDATE_COMMAND_UI(ID_OPTIONS_AUTOCOPYTOCLIPBOARD,
                     OnUpdateOptionsAutocopytoclipboard)
...
ON_UPDATE_COMMAND_UI(ID_OPTIONS_MINIMIZETOSYSTRAY,
                     OnUpdateOptionsMinimizetosystray)
...
ON_UPDATE_COMMAND_UI(ID_POPUP_SAMPLING_3BY3AVERAGE,
                     OnUpdatePopupSampling3by3average)
ON_COMMAND(ID_POPUP_SAMPLING_5BY5AVERAGE, ChangeTo5x5Sampling)
ON_UPDATE_COMMAND_UI(ID_POPUP_SAMPLING_5BY5AVERAGE,
                     OnUpdatePopupSampling5by5average)
ON_COMMAND(ID_POPUP_APPLICATION_EXPANDEDDIALOG,
           OnPopupApplicationExpandeddialog)
ON_UPDATE_COMMAND_UI(ID_POPUP_APPLICATION_EXPANDEDDIALOG,
                     OnUpdatePopupApplicationExpandeddialog)
ON_COMMAND(ID_POPUP_HEXMODE_POWERBUILDER, OnPopupHexmodePowerbuilder)
ON_UPDATE_COMMAND_UI(ID_POPUP_HEXMODE_POWERBUILDER,
                     OnUpdatePopupHexmodePowerbuilder)
ON_COMMAND(ID_POPUP_MODE_VISUALBASICHEX, OnPopupModeVisualbasichex)
ON_UPDATE_COMMAND_UI(ID_POPUP_MODE_VISUALBASICHEX,
                     OnUpdatePopupModeVisualbasichex)
...
ON_UPDATE_COMMAND_UI(ID_POPUP_APPLICATION_EASYMOVE,
                     OnUpdatePopupApplicationEasymove)
ON_COMMAND(ID_POPUP_APPLICATION_MINIMIZETOSYSTEMTRAYONSTART,
           OnPopupApplicationMinimizetosystemtrayonstart)
ON_UPDATE_COMMAND_UI(ID_POPUP_APPLICATION_MINIMIZETOSYSTEMTRAYONSTART,
                     OnUpdatePopupApplicationMinimizetosystemtrayonstart)
ON_WM_CAPTURECHANGED()
ON_COMMAND(ID_POPUP_COLOR_CONVERTTOGRAYSCALE, OnPopupColorConverttograyscale)
ON_COMMAND(ID_POPUP_APPLICATION_ALLOWMULTIPLEINSTANCES,
           OnPopupApplicationAllowmultipleinstances)
ON_UPDATE_COMMAND_UI(ID_POPUP_APPLICATION_ALLOWMULTIPLEINSTANCES,
                     OnUpdatePopupApplicationAllowmultipleinstances)
ON_COMMAND(ID_POPUP_COLOR_DETECTWEBSAFE, OnPopupColorDetectwebsafe)
ON_UPDATE_COMMAND_UI(ID_POPUP_COLOR_DETECTWEBSAFE,
                     OnUpdatePopupColorDetectwebsafe)
...
ON_COMMAND(ID_POPUP_OPTIONS_MAGNIFYWHILEEYEDROPPING,
           OnPopupOptionsMagnifywhileeyedropping)
ON_UPDATE_COMMAND_UI(ID_POPUP_OPTIONS_MAGNIFYWHILEEYEDROPPING,
                     OnUpdatePopupOptionsMagnifywhileeyedropping)
ON_COMMAND(ID_POPUP_OPTIONS_USECROSSHAIRCURSOR,
           OnPopupOptionsUsecrosshaircursor)
ON_UPDATE_COMMAND_UI(ID_POPUP_OPTIONS_USECROSSHAIRCURSOR,
                     OnUpdatePopupOptionsUsecrosshaircursor)
ON_UPDATE_COMMAND_UI(ID_POPUP_OPTIONS_STARTCURSORONEYEDROPPER,
                     OnUpdatePopupOptionsStartcursoroneyedropper)
ON_COMMAND(ID_POPUP_OPTIONS_STARTCURSORONEYEDROPPER,
           OnPopupOptionsStartcursoroneyedropper)
ON_WM_WINDOWPOSCHANGING()
ON_COMMAND(ID_POPUP_SAMPLING_DECREASEMULTIPIXELAVERAGE,
           OnPopupSamplingDecreasemultipixelaverage)
ON_COMMAND(ID_POPUP_SAMPLING_INCREASEMULTIPIXELAVERAGE,
           OnPopupSamplingIncreasemultipixelaverage)
ON_COMMAND(ID_POPUP_SAMPLING_MULTIPIXEL, OnPopupSamplingMultipixel)
ON_UPDATE_COMMAND_UI(ID_POPUP_SAMPLING_MULTIPIXEL,
                     OnUpdatePopupSamplingMultipixel)
...
BOOL CColorCopDlg::OnInitDialog() {
  ...
  // SetIcon:: Specifies a 32 pixel by 32 pixel icon if TRUE; specifies a 16
  // pixel by 16 pixel icon if FALSE.
...
void CColorCopDlg::SetupSystemMenu() {
  ...
  m_hAcceleratorTable = ::LoadAccelerators(AfxGetInstanceHandle(),
...
bool CColorCopDlg::LoadPersistentVariables() {
  ...
  hBitmap = (HBITMAP)LoadImage(AfxGetApp()->m_hInstance,
                               strBMPFile.GetBuffer(MAX_PATH), IMAGE_BITMAP, 0,
                               0, LR_LOADFROMFILE);
...
void CColorCopDlg::SetupTrayIcon() {
  if (bMinimized && (pTrayIcon_ == 0) && (m_Appflags & MimimizetoTray)) {
    ...
    pTrayIcon_->Create(0, nTrayNotificationMsg_, strAppName, hIcon_,
                       IDR_SYSTRAY_MENU);
  } else {
    delete pTrayIcon_;
    pTrayIcon_ = 0;
...
void CColorCopDlg::OnPaint() {
  if (IsIconic()) {
    ...
    //::SetForegroundWindow(CCopHWND);        // warning:        the task bar
    //: will flash if you call this
...
void CColorCopDlg::RecalcZoom() {
  ...
  if (hZoomBitmap) {
    ...
    ::StretchBlt(hdcMem,                            // destination DC
                 ...
                 ((magrect.right - magrect.left) / 2) -
                     (magwidth / 2), // x coordinate of source
                 ((magrect.bottom - magrect.top) / 2) -
                     (magheight / 2), // y coordinate of source
...
void CColorCopDlg::OnconvertRGB() {
  ...
  if (m_Appflags & ModeHTML) {
    ...
    m_Hexcolor.Format("%d",
                      (65536 * m_Bluedec) + (256 * m_Greendec) + (m_Reddec));
    ...
    wsprintf(placeh, "%%0.%df,%%0.%df,%%0.%df", m_FloatPrecision,
             m_FloatPrecision, m_FloatPrecision);
    m_Hexcolor.Format(placeh, ((float)m_Reddec) / 255.0,
                      (float)(m_Greendec) / 255.0, ((float)m_Bluedec) / 255.0);
...
*/
  ...
  if (m_Appflags & ExpandedDialog) {
    ...
    // magmod.SetRect(colorpalrect.right-9,magrect.top, colorpalrect.right,
    // magrect.top+9);

    pDC->FrameRect(magplus, &greybrush);
    pDC->MoveTo(magplus.left + 2,
                magplus.top + (magplus.bottom - magplus.top) / 2);
    pDC->LineTo(magplus.right - 2,
                magplus.top + (magplus.bottom - magplus.top) / 2);

    pDC->MoveTo(magplus.left + (magplus.right - magplus.left) / 2,
                magplus.top + 2);
    pDC->LineTo(magplus.left + (magplus.right - magplus.left) / 2,
                magplus.bottom - 2);

    // pDC->LineTo(magplus.right - 2,magplus.top +  (magplus.bottom -
    // magplus.top)/2);

    pDC->FrameRect(magminus, &greybrush);
    pDC->MoveTo(magminus.left + 2,
                magminus.top + (magminus.bottom - magminus.top) / 2);
    pDC->LineTo(magminus.right - 2,
                magminus.top + (magminus.bottom - magminus.top) / 2);
    ...
    if ((m_Appflags & MAGWHILEEYEDROP) && (m_isEyedropping) && (!m_MagDrop)) {
      ...
      if ((m_Appflags & Sampling3x3) || (m_Appflags & Sampling5x5) ||
      ...
      /*
         ...
         upper left dest magrect.Width()-4, magrect.Height()-4,  // width of
         dest rect
              }
      */
    }
...
void CColorCopDlg::OnCopytoclip() {
  if (m_Appflags &
      AutoCopytoClip) // the option to auto copy to the clipboard is ON
  {
...
void CColorCopDlg::StopCapture() {
  // we don't want to capture anymore, they let up the left mouse button, or hit
  // ESC
...
void CColorCopDlg::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (pWnd && pWnd->GetSafeHwnd() == m_EyeLoc.GetSafeHwnd()) {
    ...
    // has left clicked on the  the caption, Note: does not maximize on double
    // click from MSDN:
    //
...
HBITMAP CColorCopDlg::CopyBitmap(HBITMAP hBitmapSrc) {
  ...
  ::BitBlt(hdcDst, 0, 0, bitmap.bmWidth * 2, bitmap.bmHeight * 2, hdcSrc, 0, 0,
           SRCCOPY);
...
void CColorCopDlg::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  // OK - no more eyedropping or magnifying because we don't own the cursor
  // anymore..
...
void CColorCopDlg::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  if ((m_isEyedropping) || (m_isMagnifying)) {
    ...
    if (::GetCapture() == NULL) {
      ...
      // StopCapture();                                            // and do we
      // have capture? SetStatusBarText("Error, Lost Mouse Capture."); // if
      // not, stopcapturing.. m_isEyedropping = m_isMagnifying = FALSE;
  ...
  if (m_isEyedropping) {
    ...
    if (bRelativePosition) {
      ...
      strStatus.Format(strStatus, iWidth, iHeight, dLength, dAngle);

    } else if ((m_Appflags & Sampling3x3) || (m_Appflags & Sampling5x5) ||
               (m_Appflags & SamplingMULTI)) {

      strStatus.LoadString(IDS_EYEDROPPING);
      strStatus += ", %dx%d";
      strStatus.Format(strStatus, point.x, point.y, m_iSamplingOffset * 2 + 1,
...
BOOL CColorCopDlg::GetShellFolderPath(char *pShellFolder, char *pShellPath) {
  ...
  if (rc == ERROR_SUCCESS) {
    rc = RegQueryValueEx(hkey, pShellFolder, NULL, &type, (BYTE *)pShellPath,
                         &length);
    RegCloseKey(hkey);
  }
...
void CColorCopDlg::OnDestroy() {
  ...
  */
  if (hBitmap) {
    ...
    //        CreateBMPFile(curwindowhwnd, (LPTSTR)strBMPFile.Tostring
    //        ,MagBmpInfo,hBitmap,::GetDC(NULL));

    // working
    // CreateBMPFile(curwindowhwnd, szPath ,MagBmpInfo,hBitmap,::GetDC(NULL));

    CreateBMPFile(curwindowhwnd, strBMPFile.GetBuffer(MAX_PATH), MagBmpInfo,
                  hBitmap, ::GetDC(NULL));
...
void CColorCopDlg::OnFileExit() { EndDialog(IDOK); }
...
void CColorCopDlg::ToggleOnTop(bool bSetStatusbartext) {
  ...
  if (pSysMenu != NULL) {
    if (m_Appflags & AlwaysOnTop)
      pSysMenu->CheckMenuItem(IDM_ALWAYSONTOP,
                              MF_CHECKED); // check the menu item
    else
      pSysMenu->CheckMenuItem(IDM_ALWAYSONTOP,
                              MF_UNCHECKED); // uncheck the item
  }
}
...
void CColorCopDlg::UpdateMenu(CMenu *pMenu) {
  // this function loops through each menu item and updates the checkboxes and
  // radio buttons
...
void CColorCopDlg::OnPopupColorConverttograyscale() {
  ...
  if ((m_Reddec == m_Greendec) && (m_Greendec == m_Bluedec)) {
    ...
    // it chooses the point on the neutral axis determined by computing , which
    // corresponds to the definition of lightness. After applying Desaturate,
    // the image remains in RGB space and continues to have three color
    // channels, but now the channels have identical values, which is why the
    // image appears as a grayscale.
...
void CColorCopDlg::TestForExpand() {
  ...
  InvalidateRect(&currect,
                 false); // go redraw... but don't erase or it will flicker
}
...
bool CColorCopDlg::AveragePixelArea(HDC hdc, int *m_R, int *m_G, int *m_B,
                                    CPoint point) {
  ...
  int reddec = 0, greendec = 0,
      bluedec = 0; // temp variables to add up the values
...
void CColorCopDlg::OnPopupApplicationExpandeddialog() { OnExpandDialog(); }
...
void CColorCopDlg::OnUpdatePopupApplicationMinimizetosystemtrayonstart(
    CCmdUI *pCmdUI) {
...
void CColorCopDlg::OnUpdatePopupApplicationAllowmultipleinstances(
    CCmdUI *pCmdUI) {
...
void CColorCopDlg::FireOptionMenu() {
  ...
  // Pop up the system menu when a user hits SHIFT + F10 (right click
  // equivalent) suggested by that dude from PC Mag
  ...
  CMenu *pPopup = tempMenu.GetSubMenu(0);
  pPopup->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, targetp.x, targetp.y,
                         this, NULL);
}
...
void CColorCopDlg::CreateBMPFile(HWND hwnd, LPTSTR pszFile, PBITMAPINFO pbi,
...
bool CColorCopDlg::isWebsafeColor(int R, int G, int B) {
  ...
  return (R % WEBSAFE_STEP == 0) && (G % WEBSAFE_STEP == 0) &&
         (B % WEBSAFE_STEP == 0);
}
